...
import 'package:mobil_flutter/presentation/features/auth/screens/misafir_ekrani.dart'; // YENİ EKRANI İMPORT ET
  ...
  Widget build(BuildContext context) {
    ...
    if (girisYapildi) {
      ...
      pages = const [
        KesfetEkrani(),
        HaritaEkrani(),
        Scaffold(body: Center(child: Text("Rotalar (Yapım Aşamasında)"))), // Rotalar
        ProfilSayfasi(), // Kendi profilini gösterir
      ];
      items = [
        const BottomNavigationBarItem(icon: Icon(Icons.explore_outlined), label: 'Keşfet'),
        const BottomNavigationBarItem(icon: Icon(Icons.map_outlined), label: 'Harita'),
        const BottomNavigationBarItem(icon: Icon(Icons.route_outlined), label: 'Rotalar'),
        const BottomNavigationBarItem(icon: Icon(Icons.person_outline), label: 'Profilim'),
      ];
    } else {
